...
**解决**：用 `canvas-design` 跑一张 4×2 grid 大图（8 件主题相关物品 · 白底 · 60px breathing space · unified style），用 `extract_grid.py --mode bbox` 抠成 8 张独立透明 PNG。

**Prompt 要点**（详细 prompt patterns 见 `canvas-design` skill）：
