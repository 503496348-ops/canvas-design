...
> 基于 三支标杆产品动画（现代设计 / Hermes Agent Desktop / Claude for Word）
> 的深度拆解，提炼出的"专业级"动画设计规则。
>
...
**你是一个研究过 Apple / Pentagram / Field.io 运动档案的 motion designer。**
...
真实世界的物体有质量、有惯性、有弹性、会溢出。三支标杆片子的「高级感」根源，
...
三支标杆片子的所有动效都用带「阻尼感」的贝塞尔曲线。默认的 cubic easeOut
...
三支标杆片子没有一支用 `#FFFFFF` 或 `#000000` 做主底色。**带色温的中性色**
...
- 现代设计 展示 tweak 参数、拖滑块（不是一键生成完美结果）
- Hermes Agent 展示代码报错 + AI 修复（不是一次成功）
...
三支标杆片子的 Logo 出场**都不是简单 fade-in**，是**前一个视觉元素形变而来**。
...
- 业界标准动画拆解：设计师项目目录的 `参考动画/BEST-PRACTICES.md`
- 标杆音频拆解：同目录 `AUDIO-BEST-PRACTICES.md`
