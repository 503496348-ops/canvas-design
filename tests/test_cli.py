"""Tests for canvas-design CLI entry point."""
...
def test_help():
    ...
    assert 'canvas-design' in r.stdout.lower() or 'usage' in r.stdout.lower() or len(r.stdout) > 20
...
def test_pipeline():
    """pipeline command should show templates."""
    r = subprocess.run([sys.executable, os.path.join(SCRIPTS, 'cli.py'), 'pipeline'],
                       capture_output=True, text=True, timeout=10)
    assert r.returncode == 0
