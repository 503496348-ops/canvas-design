...
> Director's Notes for the **canvas-design v2.0** launch film
> 30 seconds · 1920×1080 · 25 fps · no voiceover
> Director: canvas-design (acting as Apple-tier launch film director)
...
这不是 slogan，是世界观。Markdown 不是「一种轻量级文档格式」——它是写作的源头。一切下游的形式（html、docx、pdf、epub）都是从这同一个源头派生出的产物。canvas-design v2.0 把这条产物链从 4 条延长到 6 条——但延长的不是「功能列表」，是**源头的影响力半径**。
...
业界把「赤陶橙 + 衬线 + 几何抽象」做成了 AI 公司的反 slop 样板。它告诉行业：你可以是科技公司，但你也可以看起来像 Penguin Classics 出版的一本哲学小书。

我们继承这套色彩。但要做得**更克制**——业界偶尔用纯赤陶橙作大色块；我们的赤陶橙永远只作 accent（占总画面 < 8% 面积），剩下 92% 留给象牙白和墨黑。
...
**主受众 A · 已使用 v1 的 canvas-design 老用户（约占 60% 流量）**
...
**次受众 B · 听说过 canvas-design 但没用的 AI Native 创作者（约 25%）**
...
Terracotta     #C2410C    主 accent（标准调）         5-8%
...
│                                  │
│  ─ atomcollide, 2026.05.11            │  ← italic 14px, Smoke
│                                  │
...
- ✅ pause-and-look signature：PDF B 顶部的「大32开 · 176×240mm」中英混排，是设计师生态对国内纸质书规格的尊重
...
```
   ...
   ║                                    ║
   ║  an essay · 设计师                   ║  ← italic 14px Smoke on Charred
   ║                                    ║
   ╠════════════════════════════════════╣
...
**首选**：用 canvas-design skill 的 `assets/sfx/<category>/*.mp3` 37 个预制资源：
